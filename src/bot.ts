...
// Функция для вычисления следующего времени публикации по правилу слотов на день
function calculateNextPublishAt(lastPublishAtIso: string | null): Date {
  // Если предыдущей даты нет, используем текущую дату
  const now = new Date();

  // Задаем фиксированные слоты публикации в часах
  const slots = [9, 12, 15, 18, 20];

  // Если предыдущая дата не передана, планируем на сегодня в первый слот (09:00)
  if (!lastPublishAtIso) {
    // Создаем новую дату на основе текущей
    const firstDate = new Date(now);
    // Устанавливаем время на первый слот дня 09:00
    firstDate.setHours(slots[0], 0, 0, 0);
    // Возвращаем вычисленную дату
    return firstDate;
  }
  ...
  // Получаем часы последней публикации
  const lastHour = lastDate.getHours();

  // Ищем индекс текущего слота в массиве слотов
  const currentSlotIndex = slots.indexOf(lastHour);

  // Если нашли слот и это не последний слот дня
  if (currentSlotIndex !== -1 && currentSlotIndex < slots.length - 1) {
    // Создаем копию даты последней публикации
    const sameDayNextSlot = new Date(lastDate);
    // Устанавливаем время на следующий слот этого же дня
    sameDayNextSlot.setHours(slots[currentSlotIndex + 1], 0, 0, 0);
    // Возвращаем вычисленную дату следующего слота
    return sameDayNextSlot;
  }

  // Во всех остальных случаях (неизвестный час или последний слот дня)
  // планируем на следующий день в первый слот (09:00)
  const nextDayFirstSlot = new Date(lastDate);
  // Увеличиваем дату на один день
  nextDayFirstSlot.setDate(nextDayFirstSlot.getDate() + 1);
  // Устанавливаем время на первый слот дня
...
bot.on('text', async (ctx) => {
  ...
  // Если пользователь нажал кнопку проверки дат, запускаем обновление статусов
  if (text === '🔍 Проверить даты') {
    try {
      // Вызываем сервис для отметки прошедших постов как опубликованных
      const updatedCount = await supabaseService.markPastPostsAsPublished();

      // Если обновленных постов нет, информируем пользователя
      if (updatedCount === 0) {
        await ctx.reply('✅ Нет постов для обновления. Все запланированные публикации ещё в будущем.');
      } else {
        // Иначе сообщаем, сколько постов было обновлено
        await ctx.reply(`✅ Обновлено постов: ${updatedCount}. Статус изменён на "опубликовано" для прошедших дат.`);
      }
    } catch (error: any) {
      // Логируем ошибку обновления статусов
      console.error('Ошибка при обновлении статусов постов:', error);
      // Отправляем пользователю сообщение об ошибке без технических деталей
      await ctx.reply('❌ Не удалось проверить даты и обновить статусы. Попробуйте позже.');
    }
